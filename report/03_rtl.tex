...
To evaluate our logic framework on realistic compiler intermediate
representations,
...
it can be related to any value $v_t$ produced the target program.
...
\begin{figure}[htp]
  ...
  \begin{tabular}{ccll}
    ...
    $\textsf{op}$
    & $\Coloneqq$
    & $\mathtt{Add} \mid \mathtt{Sub} \mid \mathtt{Mul} \mid \mathtt{Div}$
    & integer arithmetic \\
    & $\mid$
    & $\mathtt{Incr} \mid \mathtt{Decr}$
    & increment and decrement \\
    & $\mid$
    & $\mathtt{ImmInt}(z) \mid \mathtt{ImmBool}(b)$
    & constant immediate values \\
    & $\mid$
    & $\mathtt{Move} \mid \mathtt{EqZ}$
...
\begin{equation*}
  f \in \textsf{rtl\_function} \Coloneqq \left\{\,
  \begin{array}{l@{\quad}l}
    \textsf{fn\_name} \in \ident
    & \text{function identifier} \\
    \textsf{fn\_regs} \in \listT{\reg}
    & \text{parameter list} \\
    \textsf{fn\_entrypoint} \in \node
...
The code map $\textsf{fn\_code}$ is a finite map associating program points
with instructions.
In the following,
...
\begin{figure}[htb]
  \begin{gather*}
    ...
    \\[1em]
    \begin{prooftree}
      \hypo{|\vec{v}| = |\textsf{fn\_regs}(f)|}
      \hypo{\rho = \initregs{\textsf{fn\_regs}(f)}{\vec{v}}}
      \infer2[\textsc{Step-Entry}]{
        \step{P}{
          (\sigma, \CallState{f}{\vec{v}}, m)
        }{
          (\sigma, \State{f}{\textsf{fn\_entrypoint}(f)}{\rho}, m)
        }
      }
    \end{prooftree}
    \\[1em]
    ...
    \end{prooftree}
  \end{gather*}
  \caption{Small-step operational semantics of RTL.}
  \label{fig:rtl-semantics}
\end{figure}

The step relation $\step{P}{s}{s'}$ is defined by the inference rules presented
in \cref{fig:rtl-semantics}.
...
\begin{enumerate}
  ...
  \item Then we use the instruction rules to execute the body of the function.
        ...
        For instance in the \textsc{Step-Op} rule,
...
the callee is verified in isolation starting from an empty call stack
$([], \CallState{f}{\vec{v}}, m)$.
Once the callee reduces to $([], \ReturnState{v}, m')$,
\cref{lem:rtl-lift-step} lifts the derivation back into the caller's stack context,
