...
parameterized by a relation on values and memory $\vsim{}{}$:
\begin{gather*}
  \begin{prooftree}
    \hypo{\vsim{(v_t, m_t)}{(v_s, m_s)}}
...
we focus on direct program refinements $T \sqsubseteq S$.
Equipped with coinductive predicates to handle potentially infinite
executions,
we can define a simulation relation to establish such refinements.
In general,
...
A \emph{direct backward simulation} relates a target configuration $t$
...
and memories.
We formalize this condition using the predicate $\bfinalf$.
% \begin{equation}
%   \label{eq:both-final}
%   \bfinal{t}{s}
%   \coloneqq \exists v_t\, v_s\, m_t\, m_s,\;
%   \isfinal[t]{t} = \Some{v_t, m_t} \land
%   \isfinal[s]{s} = \Some{v_s, m_s} \land
%   \vsim{(v_t, m_t)}{(v_s, m_s)}
% \end{equation}
The assertion $\bfinal{t}{s}$
holds when both target and source configurations have reached final states,
producing values $v_t, v_s$ and memories $m_t, m_s$
that satisfy $\vsim{(v_t, m_t)}{(v_s, m_s)}$.
The lock-step termination condition is then expressed as:
\begin{equation}
...
is a direct \emph{lock-step} backward simulation:
...
To streamline the notation for stuttering transitions,
we write $\esim{t}{\prec i}{s}$ as syntactic sugar for:
\begin{equation*}
  \esim{t}{\prec i}{s} \coloneqq \exists i', i' \prec i \land \esim{t}{i'}{s}
\end{equation*}
...
\begin{figure}[htb]
  \begin{gather*}
    ...
    \begin{prooftree}
      \hypo{\step[s]{s}{s'}}
      \hypo{\esim{t}{\prec i}{s'}}
      \infer[coind]2[\textsc{ESourceSteps}]{\esim{t}{i}{s}}
    \end{prooftree}
    \\[1em]
    \begin{prooftree}
      \hypo{\canp[t]{t}}
      ...
      \infer[coind]2[\textsc{ETargetSteps}]{\esim{t}{i}{s}}
    \end{prooftree}
    \\[1em]
    \begin{prooftree}
      \hypo{\canp[t]{t}}
