...
*)
Class IsOp {A : ra} (a b1 b2 : A) := is_op : a = b1 ⋅ b2.
Global Arguments is_op {_} _ _ _ {_}.
Global Hint Mode IsOp + - - - : typeclass_instances.

Global Instance is_op_op {A : ra} (a b : A) : IsOp (a ⋅ b) a b | 100.
Proof. by unfold IsOp. Qed.

Class IsOp' {A : ra} (a b1 b2 : A) :=
  #[global] is_op' :: IsOp a b1 b2.
Global Hint Mode IsOp' + ! - - : typeclass_instances.
Global Hint Mode IsOp' + - ! ! : typeclass_instances.

Class IsOp'LR {A : ra} (a b1 b2 : A) := is_op_lr : IsOp a b1 b2.
Global Existing Instance is_op_lr | 0.
Global Hint Mode IsOp'LR + ! - - : typeclass_instances.
Global Instance is_op_lr_op {A : ra} (a b : A) : IsOp'LR (a ⋅ b) a b | 0.
Proof. by unfold IsOp'LR, IsOp. Qed.
