...
Abbreviation frac := Qp (only parsing).
...
Section frac.
  ...
  Proof.
    ...
    - intros x y z. unfold op, frac_op_instance. now apply Qp.add_assoc.
    - intros x y. unfold op, frac_op_instance. now apply Qp.add_comm.
    - intros p q. rewrite !frac_valid, frac_op. intros H.
      trans (p + q)%Qp; last done. apply Qp.le_add_l.
  Qed.

  Canonical Structure fracRA := Ra frac frac_ra_mixin.
  ...
  Proof. intros p1 p2 _. apply (inj (Qp.add q)). Qed.
  ...
  Proof. unfold IsOp', IsOp. by rewrite frac_op, Qp.div_2. Qed.
End frac.
