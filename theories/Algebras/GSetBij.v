...
From RSL.Algebras Require Export View GSet.
From RSL.Algebras Require Import Updates.
...
Section gset_bijective.
  ...
  Proof using Type. rewrite /gset_bijective. set_solver. Qed.
...
Section gset_bij_view_rel.
  Context `{Countable A, Countable B}.
  Implicit Types (bijL : gset (A * B)) (L : gsetURA (A * B)).

  Local Definition gset_bij_view_rel_raw bijL L: Prop :=
    L ⊆ bijL ∧ gset_bijective bijL.

  Local Lemma gset_bij_view_rel_raw_mono bijL L1 L2 :
    gset_bij_view_rel_raw bijL L1 →
    L2 ≼ L1 →
    gset_bij_view_rel_raw bijL L2.
  Proof using Type.
    intros [??] ?%gset_included.
    split; [|done]. by trans L1.
  Qed.

  Local Lemma gset_bij_view_rel_raw_valid bijL L :
    gset_bij_view_rel_raw bijL L → ✓ L.
  Proof using Type. by intros _. Qed.

  Local Lemma gset_bij_view_rel_raw_unit :
    ∃ bijL, gset_bij_view_rel_raw bijL ε.
  Proof using Type. exists ∅. split; eauto using gset_bijective_empty. Qed.

  Canonical Structure gset_bij_view_rel : view_rel (gset (A * B)) (gsetURA (A * B)) :=
    ViewRel _ _ gset_bij_view_rel_raw gset_bij_view_rel_raw_mono
            gset_bij_view_rel_raw_valid gset_bij_view_rel_raw_unit.

  Local Lemma gset_bij_view_rel_iff bijL L :
    gset_bij_view_rel bijL L ↔ L ⊆ bijL ∧ gset_bijective bijL.
...
Definition gset_bijRA A B `{Countable A, Countable B} : ra :=
  viewRA _ _ (gset_bij_view_rel (A:=A) (B:=B)).
Definition gset_bijURA A B `{Countable A, Countable B} : ura :=
  viewURA _ _ (gset_bij_view_rel (A:=A) (B:=B)).
...
Section gset_bij.
  ...
  Proof using Type.
    ...
    rewrite [(●V{dq2} L ⋅ ◯V L)]ra_comm.
    rewrite ra_assoc.
    rewrite -(ra_assoc (●V{dq1} L)).
    rewrite -(core_id_dup (◯V L)).
    rewrite -ra_assoc.
    rewrite [(◯V L ⋅ _)]ra_comm.
    by rewrite ra_assoc.
  Qed.
  ...
  Proof using Type. rewrite gset_bij_auth_dfrac_valid. naive_solver by done. Qed.
  ...
  Proof using Type. by apply gset_bij_auth_empty_dfrac_valid. Qed.

  Lemma gset_bij_auth_dfrac_op_valid dq1 dq2 L1 L2 :
    ✓ (gset_bij_auth dq1 L1 ⋅ gset_bij_auth dq2 L2)
    ↔ ✓ (dq1 ⋅ dq2) ∧ L1 = L2 ∧ gset_bijective L1.
  Proof using Type.
    rewrite /gset_bij_auth.
    rewrite (ra_comm (●V{dq2} _)) !ra_assoc.
    rewrite -(ra_assoc _ _ (◯V _)) -view_frag_op.
    rewrite ra_comm ra_assoc (ra_comm (●V {dq2} L2)).
    split.
    - move=> /ra_valid_op_l /view_auth_dfrac_op_valid.
      setoid_rewrite gset_bij_view_rel_iff. naive_solver.
  ...
  Proof using Type. rewrite gset_bij_auth_dfrac_op_valid. naive_solver. Qed.

  Lemma bij_both_dfrac_valid dq L a b :
    ✓ (gset_bij_auth dq L ⋅ gset_bij_elem a b) ↔ ✓ dq ∧ gset_bijective L ∧ (a, b) ∈ L.
  Proof using Type.
    rewrite /gset_bij_auth /gset_bij_elem -ra_assoc -view_frag_op view_both_dfrac_valid.
    setoid_rewrite gset_bij_view_rel_iff. set_solver.
  Qed.

  Lemma bij_both_valid L a b :
    ✓ (gset_bij_auth (DfracOwn 1) L ⋅ gset_bij_elem a b) ↔ gset_bijective L ∧ (a, b) ∈ L.
  Proof using Type. rewrite bij_both_dfrac_valid. naive_solver by done. Qed.
  ...
  Proof using Type.
    ...
    naive_solver eauto using subseteq_gset_bijective.
  Qed.

  Lemma bij_view_included dq L a b :
    (a,b) ∈ L → gset_bij_elem a b ≼ gset_bij_auth dq L.
  Proof using Type.
    intros. etrans; [|apply ra_included_r].
