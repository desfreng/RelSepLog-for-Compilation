...
From RSL.Algebras Require Export RA Frac Updates ProofModeClasses.
...
Notation "{ dq }" := (dq) (in custom dfrac at level 0, dq constr).
Notation "□" := DfracDiscarded (in custom dfrac).
Notation "{# q }" := (DfracOwn q) (in custom dfrac at level 0, q constr).
...
Section dfrac.
  ...
  Proof.
    ...
    - unfold included, op, dfrac_op_instance. intros [[o| |?] [= ->]]. by exists o.
  ...
  Proof.
    split; try apply _.
    - by intros [?| |?] [?| |?] [?| |?];
        unfold op, dfrac_op_instance; try rewrite Qp.add_assoc.
    - by intros [?| |?] [?| |?];
        unfold op, dfrac_op_instance; try rewrite Qp.add_comm.
    - by intros [?| |?] dq; unfold pcore, dfrac_pcore_instance; intros [= <-].
    - intros [?| |?] ? [= <-]; done.
    - intros [?| |?] [?| |?] ? [[?| |?] [=]] [= <-]; eexists _; split; try done;
        apply dfrac_discarded_included.
    - intros [q| |q] [q'| |q'];
        unfold op, dfrac_op_instance, valid, dfrac_valid_instance; auto.
  ...
  Canonical Structure dfracRA := Ra dfrac dfrac_ra_mixin.

  Global Instance dfrac_full_exclusive : Exclusive (DfracOwn 1).
  Proof.
    intros [q| |q]; unfold op, valid; simpl.
    - apply Qp.not_add_le_l.
    - intros H. inv H.
    - intros H%Qp.lt_le_incl. by eapply Qp.not_add_le_l.
  Qed.
  ...
  Proof. rewrite ra_comm. apply dfrac_valid_own_r. Qed.
  ...
  Proof. unfold IsOp', IsOp. rewrite dfrac_op_own. now intros ->. Qed.
  ...
  Proof.
    ...
    - intros [qd Hqd]%Qp.lt_sum. exists (DfracOwn (qd/2)%Qp).
      split; first by eexists. simpl.
      unfold op, valid, ra_op, ra_valid; simpl.
