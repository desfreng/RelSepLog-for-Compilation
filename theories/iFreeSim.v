...
From RSL Require Import Commons.Fixpoints.

Section FSimDef.
  Context {Λₜ Λₛ: lang}.
  Context (J I: WfRel).
  Context (Pₜ: prog Λₜ) (Pₛ: prog Λₛ) (S: state Λₜ -> state Λₛ -> rlogic).
  ...
  Definition both_final (ϕ: post) (t: state Λₜ) (s: state Λₛ) : rlogic :=
    ∃ vₜ vₛ, ⌜is_final t = Some vₜ⌟ ∗ ⌜is_final s = Some vₛ⌟ ∗ ϕ vₜ vₛ.

  Local Definition fsim_body (gfp: sim_rel) (lfp: sim_rel) : sim_rel :=
    fun ϕ t j i s =>
    ((                          (* Related *)
      S t s -∗ both_final ϕ t s
    ) ∨ (                       (* Source Stuck *)
      ⌜stuck Pₛ s⌝
    ) ∨ (                       (* Source Steps *)
      S t s -∗
      ∃ s' i',
        ⌜Pₛ ⊨ s ->> s'⌟ ∗ S t s' ∗ lfp ϕ t j i' s'
    ) ∨ (                       (* Target Steps *)
      ⌜can_progress Pₜ t⌟ ∗
      (∀ t',
         ⌜Pₜ ⊨ t ->> t'⌟ -∗
  ...
  Proof using Type.
    iIntros "#Hgfp #Hlfp" (ϕ t j i s) "Hsim".
    unfold fsim_body.
    iDestruct ("Hsim")
    ...
    - do 2 iRight. iLeft.
      iIntros "H".
      iDestruct ("Hs" with "H") as (s' i') "(Hstep & Hstate & Hsim)".
      iExists s', i'. iFrame.
      iApply ("Hlfp" with "Hsim").
    - do 3 iRight. iLeft. iFrame.
      iIntros "%t' H Hstep".
      iDestruct ("Ht" $! t' with "H Hstep") as "(Hstate & [%j' Hsim])".
      iFrame. iExists j'.
      iApply ("Hlfp" with "Hsim").
    - do 4 iRight.
  ...
  Local Lemma fsim_body_mono_strong gfp1 gfp2 lfp1 lfp2:
    ⊢ □ (∀ ϕ ϕ' t j j' i i' s,
      ...
      fsim_body gfp2 lfp2 ϕ' t j' i' s.
  Proof using Type.
    iIntros "#Hgfp #Hlfp" (ϕ ϕ' t j j' i i' s) "#Hpost %Hj %Hi Hsim".
    unfold fsim_body.
    iDestruct ("Hsim") as "[Hfin | [Hstuck | [Hs | [(Hprog & Ht) | Hprog]]]]".
    - do 0 iRight. iLeft. iIntros "H".
      iDestruct ("Hfin" with "H") as "(%vt & %vs & Ht & Hs & Hfin)".
  ...
  Proof using Type. exact: seal_eq. Qed.
  ...
  Proof using Type.
    iIntros (vₜ vₛ Ht Hs) "HΦ".
    rewrite fsim_lfp_unfold.
    do 0 iRight. iLeft.
    iIntros "H". iSpecialize ("HΦ" with "H").
    iExists vₜ, vₛ.
 ...
 Qed.
  ...
  Lemma fsim_source_steps G ϕ t j i s :
    ⊢ (S t s -∗
       ∃ s' i', ⌜Pₛ ⊨ s ->> s'⌟ ∗ S t s' ∗ fsim_lfp G ϕ t j i' s') -∗
    fsim_lfp G ϕ t j i s.
  Proof using Type.
    iIntros "Hs".
    rewrite fsim_lfp_unfold.
    do 2 iRight. now iLeft.
  Qed.
  ...
  Lemma fsim_progress G ϕ t j i s:
    ...
    j' ⊏ j ->
    i' ⊏ i ->
    ⊢ G ϕ t j' i' s -∗ fsim_lfp G ϕ t j i s.
  Proof using Type.
    iIntros (j' i' Hj Hi) "Hf".
    rewrite fsim_lfp_unfold.
    do 4 iRight.
    iExists j', i'. iFrame. by iSplit.
  Qed.

  Local Lemma fsim_lfp_strong_ind (P R: sim_rel):
    ⊢ (□ ∀ ϕ t j i s,
  ...
  Lemma fsim_lfp_ind (P R: sim_rel):
    ⊢ (* Related *)
      (□ ∀ ϕ t j i s,
        (S t s -∗ both_final ϕ t s) -∗ P ϕ t j i s) -∗
      (* Source Stuck *)
      (□ ∀ ϕ t j i s,
        ⌜stuck Pₛ s⌝ -∗ P ϕ t j i s) -∗
      (* Source Steps *)
      (□ ∀ ϕ t j i s,
         (S t s -∗
          ∃ s' i',
            ⌜Pₛ ⊨ s ->> s'⌟ ∗
            S t s' ∗
            P ϕ t j i' s' ∧ fsim_lfp R ϕ t j i' s') -∗
         P ϕ t j i s) -∗
      (* Target Steps *)
      (□ ∀ ϕ t j i s,
        (⌜can_progress Pₜ t⌟ ∗
         (∀ t',
            ⌜Pₜ ⊨ t ->> t'⌟ -∗
            S t s -∗
            S t' s ∗ ∃ j', P ϕ t' j' i s ∧ fsim_lfp R ϕ t' j' i s)
        ) -∗
        P ϕ t j i s) -∗
      (* Progress *)
      (□ ∀ ϕ t j j' i i' s,
        ⌜j' ⊏ j⌟ -∗ ⌜i' ⊏ i⌟ -∗ R ϕ t j' i' s -∗ P ϕ t j i s) -∗
      (* Conclusion *)
      ∀ ϕ t j i s, fsim_lfp R ϕ t j i s -∗ P ϕ t j i s.
  Proof using Type.
    iIntros "#IHfin #IHstuck #IHsstep #IHtstep #IHprog" (ϕ t j i s) "H".
    iApply (fsim_lfp_strong_ind with "[] H").
    clear ϕ t j i s. iIntros "!>" (ϕ t j i s) "H".
    iDestruct ("H") as "[Hfin | [Hstuck | [Hs | [(H & Ht) | H]]]]".
    - by iApply ("IHfin" with "Hfin").
    - by iApply ("IHstuck" with "Hstuck").
    - by iApply ("IHsstep").
    - iApply ("IHtstep"). by iFrame.
    - iDestruct "H" as (j' i') "(Hj & Hi & HR)".
      by iApply ("IHprog" with "Hj Hi HR").
  Qed.
  ...
  Proof using Type.
    iIntros "#Hgfp Hsim".
    iApply (fsim_lfp_strong_ind with "[] Hsim"); clear.
  ...
  Local Definition fsim_lfp_mono_inv R ϕ t j i s : rlogic :=
    ∀ ϕ' j' i',
    □ (∀ vt vs, ϕ vt vs -∗ ϕ' vt vs) -∗
    ⌜j ⊑ j'⌟ -∗
    ⌜i ⊑ i'⌟ -∗
  ...
  Proof using Type.
    ...
    {
      iApply (fsim_lfp_strong_ind (fsim_lfp_mono_inv gfp') with "[] Hsim").
      clear.
      iIntros "!>" (ϕ t j i s) "IH".
  ...
  Proof using Type. unfold fsim. by rewrite (seal_eq fsim_aux). Qed.

  Lemma fsim_fixpoint ϕ t j i s:
    fsim ϕ t j i s ⊣⊢ fsim_lfp fsim ϕ t j i s.
  Proof using Type. rewrite fsim_eq. by apply fsim_gfp_def_unfold. Qed.
  ...
  Local Definition fsim_mono_coind_inv ϕ' t j' i' s : rlogic :=
    ∃ ϕ j i,
      □ (∀ vt vs, ϕ vt vs -∗ ϕ' vt vs) ∗
      ⌜j ⊑ j'⌟ ∗
      ⌜i ⊑ i'⌟ ∗
