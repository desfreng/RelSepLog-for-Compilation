...
Section least.
  ...
  Proof using Type*.
    ...
    iIntros "HF" (Φ) "#Hincl".
    iApply "Hincl".
    iApply (bi_mono_pred _ Φ with "[#] HF").
 ...
 Lemma least_fixpoint_persistent_affine :
  ...
  Proof using Type*.
    intros ?? x. rewrite /Persistent -bi.intuitionistically_into_persistently_1.
...
Section least_ind.
  ...
  Proof using Type*.
    split.
    intros Ψ Ψ'.
    iIntros "#Mon" (x) "Ha". iSplit; first by iDestruct "Ha" as "[$ _]".
    iDestruct "Ha" as "[_ Hr]". iApply (bi_mono_pred with "[] Hr").
    by iModIntro.
  Qed.
  ...
  Proof using Type*.
    ...
    iIntros "!> %y Hy".
    iApply "Hmon".
    iApply (bi_mono_pred with "[] Hy").
    iIntros "!> %x Hx".
    iSplit.
    - rewrite least_fixpoint_unfold. iDestruct "Hx" as "[$ _]".
    - iApply (least_fixpoint_strong_mono with "[] Hx").
      iIntros "!>" (??) "[_ $]".
  Qed.
End least_ind.
...
Section greatest.
  ...
  Proof using Type*.
    iIntros "HF". iExists (F (bi_greatest_fixpoint F)).
